 ...
 */
function charmed_add_meta_box( $meta_box ) {
	if ( ! is_array( $meta_box ) ) {
		return false;
	}

	add_meta_box( $meta_box['id'], $meta_box['title'], 'charmed_metabox_callback', $meta_box['page'], $meta_box['context'], $meta_box['priority'], $meta_box );
}
 ...
 * @param array   $meta_box With metabox id, title, callback, and args elements.
 */
function charmed_metabox_callback( $post, $meta_box ) {
	return charmed_create_meta_box( $post, $meta_box['args'] );
}
 ...
 * @param array   $meta_box With metabox id, title, callback, and args elements.
 */
function charmed_create_meta_box( $post, $meta_box ) {
	// VERSION FALLBACK.
	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		echo '<tr><th><label for="' . esc_attr( $field['id'] ) . '" class="' . esc_attr( $field['id'] ) . '"><strong>' . esc_html( $field['name'] ) . '</strong>
			  <span>' . $field['desc'] . '</span></label></th>';

		switch ( $field['type'] ) {
			case 'text':
				echo '<td><input type="text" name="bean_meta[' . esc_attr( $field['id'] ) . ']" id="' . esc_attr( $field['id'] ) . '" value="' . ( $meta ? $meta : esc_attr( $field['std'] ) ) . '" size="30" /></td>';
				break;

			case 'textarea':
				echo '<td><textarea name="bean_meta[' . esc_attr( $field['id'] ) . ']" id="' . esc_attr( $field['id'] ) . '" rows="7" cols="5">' . ( $meta ? $meta : esc_attr( $field['std'] ) ) . '</textarea></td>';
				break;
				...
				jQuery(function($) {
					...
					$('#<?php echo esc_js( $field['id'] ); ?>_button').on('click', function(e) {
						...
						frame.toolbar.get('view').set({
							insert: {
								style: 'primary',
								text: '<?php esc_html_e( 'Insert', 'charmed-pro' ); ?>',
			...
			<?php
				...
				// VERSION COMPARE.
				echo '<td><input type="text" name="bean_meta[' . esc_attr( $field['id'] ) . ']" id="' . esc_attr( $field['id'] ) . '" value="' . ( $meta ? $meta : esc_attr( $field['std'] ) ) . '" size="30" class="file" /> <input type="button" class="button" name="' . $field['id'] . '_button" id="' . $field['id'] . '_button" value="Browse" /></td>';
				break;
							...
							insert: {
								...
								click: function() {
									var models = frame.state().get('selection'),
										...
										this.el.innerHTML = '<?php esc_html_e( 'Saving...', 'charmed-pro' ); ?>';
										...
										$.ajax({
											...
											data: {
												id: id,
												action: 'charmed_grid_feat_image_save',
			...
			?>
				...
				jQuery(function($) {
					...
					$('#bean_images_upload').on('click', function(e) {
						...
						//FIRST FRAME
						var options = {
							title: '<?php esc_html_e( 'Create Gallery', 'charmed-pro' ); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						//IF FRAME IS THERE
						if( frame || selection ) {
							options['title'] = '<?php esc_html_e( 'Edit Gallery', 'charmed-pro' ); ?>';
						}

						frame = wp.media(options).open();

						// CUSTOMIZE VIEWS
						frame.menu.get('view').get('cancel').el.innerHTML = '<?php esc_html_e( 'Cancel', 'charmed-pro' ); ?>';
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php esc_html_e( 'Edit Gallery', 'charmed-pro' ); ?>';
						...
						frame.state().get('library').on( 'remove', function() {
							...
							if(models.length == 0){
								selection = false;
								$.post(ajaxurl, { ids: '', action: 'charmed_image_save', post_id: bean_ajax.post_id, nonce: bean_ajax.nonce });
							}
						});
						...
						function overrideGalleryInsert() {
							frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php esc_html_e( 'Save Gallery', 'charmed-pro' ); ?>',
			...
			$thumbnail_output = '';
			$button_text      = ( $meta ) ? __( 'Edit Gallery', 'charmed-pro' ) : $field['std'];
			if ( $meta ) {
				$field['std']     = __( 'Edit Gallery', 'charmed-pro' );
				$thumbs           = explode( ',', $meta );
				$thumbnail_output = '';
			...
			case 'checkbox':
				...
				<input type="checkbox" id="' . $field['id'] . '" name="bean_meta[' . $field['id'] . ']" value="on"' . $val . ' /><span>' . __( 'Yes, please do', 'charmed-pro' ) . '</span>';
				echo '</td>';
				break;
		}
 ...
 * 3. SAVING META BOXES
 */
function charmed_save_meta_box( $post_id ) {
...
add_action( 'save_post', 'charmed_save_meta_box' );
...
add_action( 'wp_ajax_charmed_image_save', 'charmed_image_save' );

/**
 * 5. SAVING GRID FEATURE IMAGE
 */
function charmed_grid_feat_image_save() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
...
add_action( 'wp_ajax_charmed_grid_feat_image_save', 'charmed_grid_feat_image_save' );
 ...
 * 6. SCRIPTS
 */
function charmed_metabox_portfolio_scripts() {
	global $post;
	$wp_version = get_bloginfo( 'version' );
...
add_action( 'admin_enqueue_scripts', 'charmed_metabox_portfolio_scripts' );
